...
- **GC Roots**:重中之重
  ...
  - 临时组成：除了上面固定的GC Roots外，还可能把其他对象临时加入到GC Roots集合中
    - 分代收集:看下面清除阶段的算法
    - 局部回收:和下面那个小技巧同理，如果仅对堆内一部分A进行垃圾回收，如果 剩下的堆内其他部分+堆外GCRoots 中有指向A内对象的引用，那么那些对象就不能回收
      ```
...
- Parallel Scavenge
  > 回收新生代
  - 使用
    - Parallel Scavenge 和Parallel Old 两个是jdk8中的默认垃圾回收器
...
##### 2.3.3.4.9. CMS回收器：低延迟

##### 2.3.3.4.10. G1回收器：区域化分布式
