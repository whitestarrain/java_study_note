...
- 运行原理-栈帧弹出
  ...
  - Java方法有两种返回函数的方式，不管使用哪种方式，都会导致栈帧被弹出。
    ...
    - 另外一种是抛出异常。
      > 在此处深入理解异常抛出，以之前的图为例，假如方法4出现异常没有处理，方法4栈帧抛出异常，然后对应栈帧弹出
...
- 内存分配
  ...
  - 在方法运行期间不会改变局部变量表的大小。
    ...
    ```
  - 示例：
    > 反编译（javap）一个测试类的字节码文件
    > ![local-variables-1](./image/local-variables-1.png) 
    > 使用javap指令也可以查看。只是使用jclasslib结构更清晰
